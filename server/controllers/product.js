...
exports.update=async(req,res)=>{
    try{
        ...
        let updated=await Product.findOneAndUpdate({slug:req.params.slug},
            req.body,
            {new:true} ).exec()
        res.json(updated)
    }
...
// check if currently logged in user have already added rating to this product?
    ...
    if(existingRatingObject===undefined){
        ...
        // if usre have already left rating, update it
...
try{        const products=await Product.find({shipping:shipping})
        .populate("category","_id name")
        .populate("subs","_id name")
        .exec();
        res.json(products)
